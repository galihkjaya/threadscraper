...
from threads_scraper.config import BASE_HEADERS, BASE_PAYLOAD, COOKIES, DELAY_MIN, DELAY_MAX
...
def fetch_post_replies(
    ...
    Args:
        ...
        _session_ref: Single-element list holding the session so cli.py can
    ...
    try:
        ...
        for edge in (edges or []):
            ...
            for i, item in enumerate(thread_items or []):
                ...
                row = {
                    "post_code":    post_code,  # original shortcode
                    "post_id":      post_id,    # numeric media ID
                    "post_text":    "",          # filled in cli.py
